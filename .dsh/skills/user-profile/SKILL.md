...
- **两段式推送命令**（2026-09-17 起，固化上面的确认流程）：
  1. 用户说「**推送列表**」→ 只读展示：未提交改动（git status + diff --stat
     分组摘要）+ 未推送提交（log origin/main..main）；board_cache.json 等
     数据缓存噪声文件标注排除；**不执行任何写操作**
  2. 用户说「**推送github**」→ 才执行：add（排除噪声）→ 约定式提交 →
     密钥扫描自检 → push → 回报提交号。无改动时两命令都回「无待推送内容」
- **拉取命令**（2026-09-19 起）：用户说「**拉取workspace**」（或「拉取」/
  「同步最新的workspace/工作空间」）→ ① `git pull`（沙箱内 ssh 命名管道被拒
  时提权重试）② 有新提交时**必须重读本 skill 文件**（规则可能已变更——
  2026-09-15 的流程违规正源于同步后未重读）③ 有 skill 目录变更时重新加载
  受影响 skill ④ 汇报：新提交摘要 + profile/skill 变更点 + 涉及的项目文件
  ⑤ 检出新旧文档状态滞后（如需求状态行与代码不符）时顺手修正并本地提交
...
- **上下文/工作日志不入库**（2026-09-15 起）：worklog-*.md 等会话沉淀文件
  只留本地（已 gitignore），不进 git、不推送
...
- 2026-09-19 新增「拉取workspace」标准命令：pull + 重读 profile + 重载受影响
  skill + 变更汇报 + 文档状态滞后顺手修正——把 09-15 复盘第②条从"要求"
  升级为"命令化流程"，双机协作不再依赖自觉
- 2026-09-19 WPS 后台全家桶清理（workspace\disable_wps_bg_v2.ps1）：WPS 装机
  注册 3 个计划任务（登录唤醒/更新×2）+ wpscloudsvr 服务，后台进程有看门狗
  自愈，只杀进程无效。经验：①提权脚本必须自检 ELEVATED——用户两次开的都是
  普通窗口（UAC 未确认），症状与 ACL 硬化完全同形，先怀疑提权再看 ACL；
  ②正确顺序=禁任务+禁服务→杀进程→不复活；③回滚命令在脚本头注释
